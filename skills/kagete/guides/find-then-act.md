...
| `PERMISSION_DENIED` | System privacy gate | `kagete doctor --prompt`, then user approves **the host process** (terminal / harness, *not* kagete) in System Settings |
| Keystrokes vanish | Target app isn't frontmost *and* command relies on global focus (e.g. menu shortcuts) | Sequence `kagete activate --app X` before the input command |
