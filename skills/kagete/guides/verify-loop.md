...
| `click-at` / `move` | *nothing implicit* | No verify block — follow up with `find` / `screenshot` / `inspect` to confirm the click target |
...
- **Trusting exit codes alone.** `kagete type`/`key`/`click-at` return `ok:true` whenever the event was *posted*, not when it was *received*. The app may have swallowed it. Always check the envelope's post-action signal (`typeCheck.textLanded`, `valueMatches`, or a follow-up `find`).
